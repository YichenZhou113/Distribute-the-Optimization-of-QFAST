...
logger = logging.getLogger( "qfast_step" )

task_index  = int( os.environ['SLURM_PROCID'] )
logger.info( "task_index:%d" % task_index )
#print("task_index: ", task_index)
n_tasks     = int( os.environ['SLURM_NPROCS'] )
logger.info("n_tasks: %d" % n_tasks)
...
def fixed_depth_exploration ( target, num_qubits, gate_size, fun_vals,
    ...
    if rank != 0:
       ...
       # with tf.device(tf.train.replica_device_setter(ps_tasks = n_cpus)):
        ...
        with tf.device("/job:localhost/replica:0/task:0/device:CPU:0"):
            layers.append(GenericGate( "Gate%d" % 0, num_qubits, gate_size, lm,
                                fun_vals[0], loc_vals[0],
        ...
        for i in range(1, len(fun_vals)):
            ...
            with tf.device("/job:localhost/replica:0/task:%d/device:CPU:0" % (i // each_num)):
                layers.append(GenericGate( "Gate%d" % i, num_qubits, gate_size, lm,
        ...
        with tf.Session(server.target, config=tf.ConfigProto(device_count={ "CPU": n_cpus }, inter_op_parallelism_threads=n_cpus, intra_op_parallelism_threads=1,)) as sess:
            ...
            while ( True ):
                ...
                if loss < exploration_distance:
                    ...
                    result = ( True, [ l.get_fun_vals( sess ) for l in layers ], [ l.get_loc_vals( sess ) for l in layers ] )
                    q_result = []
                ...
                if len( loss_values ) > 100:
                    ...
                    if max_value - min_value < learning_rate / 10:
                        ...
                        result = ( False, [ l.get_fun_vals( sess ) for l in layers ], [ l.get_loc_vals( sess ) for l in layers ] )
                        q_result = []
                        for r in range(n_cpus):
                            if r==0: 
                                continue
    ...
    #                print("   ", node.node_name)"
            ...
            for rank_ in range(n_cpus):
                if rank_ == 0:
                    ...
                    req = comm.isend(result, dest=rank_, tag=11)
                    req.wait()
